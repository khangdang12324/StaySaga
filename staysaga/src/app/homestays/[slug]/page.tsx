...
export default async function HomestayDetailPage({ params }: Props) {
  ...
  const { data: homestay, isMock } = await getPropertyBySlug(resolvedParams.slug)
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-zinc-950">
      ...
      <main className="pt-24 pb-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Title Section */}
        <div className="mb-8">
          <h1 className="text-3xl md:text-4xl font-extrabold text-gray-900 dark:text-white mb-4">{homestay.title}</h1>
          <div className="flex flex-wrap items-center gap-4 text-sm font-medium text-gray-600 dark:text-gray-300">
            ...
            <div className="flex items-center gap-1">
              ...
              <span className="underline cursor-pointer">{homestay.location}, Việt Nam</span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 h-[50vh] min-h-[400px] mb-12 rounded-3xl overflow-hidden">
          ...
          <div className="grid grid-cols-2 grid-rows-2 gap-4 h-full hidden md:grid">
            {subImages.map((img, i) => (
              <img key={i} src={img} className="w-full h-full object-cover hover:opacity-95 transition-opacity cursor-pointer" alt={`Room ${i}`} />
            ))}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          <div className="lg:col-span-2">
            <div className="flex justify-between items-center pb-8 border-b border-gray-200 dark:border-zinc-800">
              <div>
                <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">Toàn bộ biệt thự, chủ nhà Nguyễn Văn A</h2>
                <p className="text-gray-600">8 khách · 4 phòng ngủ · 4 giường · 3 phòng tắm</p>
              </div>
              <div className="w-14 h-14 bg-gray-200 rounded-full overflow-hidden">
                <img src="https://ui-avatars.com/api/?name=Nguyen+A&background=random" alt="Host" />
              </div>
            </div>
            ...
            <div className="py-8 border-b border-gray-200 dark:border-zinc-800 space-y-6">
              <div className="flex gap-4">
                ...
                <div>
                  ...
                  <p className="text-gray-500">Chủ nhà siêu cấp là những người có kinh nghiệm, được đánh giá cao và cam kết mang lại kỳ nghỉ tuyệt vời cho khách.</p>
                </div>
              </div>
              <div className="flex gap-4">
                ...
                <div>
                  ...
                  <p className="text-gray-500">100% khách gần đây đã xếp hạng 5 sao cho vị trí này.</p>
                </div>
              </div>
            </div>
            ...
            <div className="py-8 border-b border-gray-200 dark:border-zinc-800">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="flex items-center gap-3"><Wifi className="w-6 h-6 text-gray-400" /> <span>Wifi tốc độ cao</span></div>
                <div className="flex items-center gap-3"><Wind className="w-6 h-6 text-gray-400" /> <span>Điều hòa nhiệt độ</span></div>
                <div className="flex items-center gap-3"><Car className="w-6 h-6 text-gray-400" /> <span>Bãi đỗ xe miễn phí</span></div>
                <div className="flex items-center gap-3"><Coffee className="w-6 h-6 text-gray-400" /> <span>Bếp đầy đủ tiện nghi</span></div>
              </div>
            </div>
          </div>
