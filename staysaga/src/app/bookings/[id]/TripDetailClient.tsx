...
type TripDetailClientProps = {
  ...
  lang: string;
  currency: string;
  cancelAction: (bookingId: string) => Promise<{ success?: boolean; error?: string }>;
...
export default function TripDetailClient({
  ...
  const copyToClipboard = (text: string, label: string) => {
    ...
    toast.success(`${label} ${t("đã được sao chép vào bộ nhớ tạm", "copied to clipboard")}`);
  };

  const listBookings = bookingsInCity.length > 0 ? bookingsInCity : [booking];

  const city = booking.homestay?.city || booking.homestay?.location || "TP. Hồ Chí Minh";
  const mainCheckIn = booking.check_in_date ? new Date(booking.check_in_date) : new Date();
  const mainCheckOut = booking.check_out_date ? new Date(booking.check_out_date) : new Date();
  ...
  const earliestCheckIn = sortedDates[0]?.check_in_date ? new Date(sortedDates[0].check_in_date) : mainCheckIn;
  const latestCheckOut = sortedDates[sortedDates.length - 1]?.check_out_date ? new Date(sortedDates[sortedDates.length - 1].check_out_date) : mainCheckOut;

  const nights = booking.nights || differenceInDays(mainCheckOut, mainCheckIn) || 1;
  const displayCode = booking.booking_code || booking.id?.toString().slice(0, 8).toUpperCase() || "BK-123456";
  const pinCode = "7495";
  const imgUrl = booking.homestay?.homestay_images?.[0]?.url || "/images/fallback-hotel.jpg";
  ...
  async function handleCancelConfirm(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.error) {
        ...
        toast.success(t("Hủy đặt phòng thành công!", "Booking cancelled successfully!"));
        setShowCancelModal(false);
        router.refresh();
      }
    } catch (err) {
      console.error(err);
      toast.error(t("Có lỗi xảy ra khi hủy.", "An error occurred during cancellation."));
    } finally {
    ...
    switch (status) {
      case "CONFIRMED":
        return {
          ...
          desc: t("Đơn đặt của bạn đã được chỗ nghỉ xác nhận.", "Your booking is confirmed by the property.")
        };
      case "PENDING":
        return {
          label: t("Đã xác nhận", "Confirmed"),
      ...
      case "CANCELLED":
        return {
          ...
          desc: t("Đơn đặt phòng này đã được hủy thành công.", "This booking has been successfully cancelled.")
        };
      case "COMPLETED":
        return {
          label: t("Đã hoàn tất", "Completed"),
          bg: "bg-blue-50 text-blue-700 border-blue-100",
          desc: t("Kỳ nghỉ của bạn đã hoàn tất. Cảm ơn bạn đã đồng hành cùng StaySaga!", "Your stay has completed. Thank you for choosing StaySaga!")
        };
      case "NO_SHOW":
        return {
          label: t("Không đến nhận phòng", "No Show"),
          bg: "bg-slate-100 text-slate-600 border-slate-200",
          desc: t("Bạn đã không đến nhận phòng như lịch đặt.", "You did not show up for your check-in.")
        };
      default:
        return {
          label: status,
    ...
    <div className="min-h-screen bg-[#f8fafc] text-slate-800 pb-20 font-sans">
      ...
      <div className="py-4">
        <div className="max-w-6xl mx-auto px-4 flex items-center justify-between text-xs sm:text-sm">
          <div className="flex items-center gap-2">
            <Link href="/bookings" className="text-rose-600 hover:text-rose-750 font-bold transition-colors">
              {t("Đặt chỗ & Chuyến đi", "Trips & Bookings")}
            </Link>
            <ChevronRight className="w-4 h-4 text-slate-400 shrink-0" />
            <span className="text-slate-500 font-medium truncate">{city}</span>
          </div>
          <Link href="/help" className="text-rose-600 hover:text-rose-750 font-semibold hover:underline transition-all">
      ...
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="h-48 sm:h-56 w-full relative rounded-xl overflow-hidden shadow-sm mb-6 border border-slate-200 animate-fade-in">
          <SafeImage src={getLocationImage(city)} alt={city} fill className="object-cover" />
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center p-6 text-white text-center">
            <div className="flex items-center justify-center gap-2">
              ...
              <button className="h-7 w-7 rounded-full bg-white flex items-center justify-center shadow hover:bg-slate-100 transition-colors text-slate-700 cursor-pointer" aria-label="Edit destination">
        ...
        {listBookings.map((b: any) => {
          const bHotelName = b.homestay?.name || b.homestay?.title || "StaySaga Homestay";
              ...
              <div className="p-6 flex flex-col sm:flex-row gap-5 items-start relative">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-slate-500 mt-1 flex flex-wrap items-center gap-1.5 leading-relaxed font-semibold">
                    ...
                    <span>{city}</span>
                    <span>·</span>
                    <span className="text-emerald-700 font-bold">{t("Miễn phí hủy phòng", "Free cancellation")}</span>
                  </p>
                  ...
                  <div className="mt-2.5">
                    {b.status === "CONFIRMED" || b.status === "PENDING" ? (
                      ...
                      <span className="inline-flex items-center gap-1 bg-blue-50 text-blue-700 border border-blue-100 text-xs font-bold px-2 py-0.5 rounded">
                  ...
                  <div className="text-xs text-slate-500 mt-2.5 font-semibold">
                    {t("• Trả phòng:", "• Check-out:")} {lang === "EN" ? format(bCheckOutDate, "EEEE, d MMM") : `Thứ ${format(bCheckOutDate, "i") === "1" ? "Nhật" : Number(format(bCheckOutDate, "i")) + 1}, ${format(bCheckOutDate, "d")} thg ${format(bCheckOutDate, "M")}`} {t("trước 12:00", "before 12:00")}
                  </div>
                </div>
                ...
                <div className="flex flex-col items-end gap-1.5 self-stretch sm:self-auto shrink-0 mt-3 sm:mt-0 ml-auto justify-between sm:justify-start">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="relative">
                      <button
                        onClick={() => setOpenMenuBookingId(openMenuBookingId === b.id ? null : b.id)}
                      ...
                      {openMenuBookingId === b.id && (
                        <>
                          <div className="fixed inset-0 z-10" onClick={() => setOpenMenuBookingId(null)} />
              ...
              <div className="divide-y divide-slate-100 border-t border-slate-100">
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <span>{t("Liên hệ Dịch vụ Khách hàng", "Contact Customer Service")}</span>
                  </div>
                  <ChevronRight className="w-5 h-5 text-slate-400 shrink-0" />
                </Link>
              </div>
      ...
      {showCancelModal && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-50 animate-fade-in font-sans">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden border border-slate-200">
            <div className="p-6">
              <h3 className="text-lg sm:text-xl font-extrabold text-slate-950 mb-2">{t("Hủy đặt phòng này?", "Cancel this booking?")}</h3>
              ...
              <form onSubmit={handleCancelConfirm} className="flex justify-end gap-3 border-t border-slate-100 pt-4">
                <button
                  type="button"
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden border border-slate-200">
            <div className="p-6">
              <div className="flex justify-between items-center mb-4 pb-3 border-b border-slate-100">
                <h3 className="text-lg font-bold text-slate-900">{t("Thay đổi ngày lưu trú", "Change stay dates")}</h3>
                <button 
