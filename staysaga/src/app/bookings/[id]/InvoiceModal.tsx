  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (res?.error) {
        ...
        toast.success(t("Gửi yêu cầu hóa đơn thành công!", "Invoice request sent successfully!"));
        if (res?.warning) {
    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-50 animate-fade-in font-sans">
      <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full overflow-hidden border border-slate-200 flex flex-col max-h-[90vh]">
        ...
        <div className="flex justify-between items-center p-5 border-b border-slate-100 shrink-0">
          <h3 className="text-lg font-bold text-slate-900">{t("Yêu cầu hóa đơn", "Request invoice")}</h3>
          <button
            onClick={onClose}
        ...
        <div className="p-6 overflow-y-auto space-y-6">
          ...
          <div className="flex flex-col gap-4">
            <a
              ...
              className="w-full border-2 border-blue-600 bg-white hover:bg-blue-50/10 text-blue-600 font-bold py-3 px-4 rounded-xl text-sm flex items-center justify-center gap-2 transition-all shadow-sm cursor-pointer"
            >
            ...
            <a
              ...
              className="text-blue-600 hover:text-blue-800 hover:underline font-bold text-sm text-center py-2 flex items-center justify-center gap-2 transition-all self-center cursor-pointer"
            >
              <Phone className="w-4 h-4 text-blue-500" />
              {t(`Gọi chỗ nghỉ (${hostPhone || "+842836225811"})`, `Call property (${hostPhone || "+842836225811"})`)}
            </a>
          </div>

          <div className="relative flex py-2 items-center">
            <div className="flex-grow border-t border-slate-100"></div>
            <span className="flex-shrink mx-4 text-xs font-bold text-slate-400 uppercase tracking-wider">{t("Hoặc điền thông tin", "Or fill info")}</span>
            <div className="flex-grow border-t border-slate-100"></div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input
                type="text"
                name="companyName"
                placeholder={t("Cụm công ty, tổng công ty...", "Company name...")}
            ...
            <div>
              ...
              <input
                type="text"
                name="billingAddress"
                placeholder={t("Địa chỉ đăng ký kinh doanh...", "Business registration address...")}
                className="w-full bg-white border border-slate-200 rounded-lg px-3.5 py-2 text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-rose-500 focus:border-rose-500 transition-all placeholder-slate-400"
              />
            </div>
            ...
            <div>
              ...
              <textarea
                name="note"
                rows={2}
                placeholder={t("Ví dụ: xuất trước ngày 25...", "E.g. issue before 25th...")}
