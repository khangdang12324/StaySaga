...
sections.push([
  ...
  normalText(
    "Phần này trình bày các đoạn mã nguồn chính của hệ thống StaySaga.",
  ),
  ...
  normalText(
    "Đoạn mã này cấu hình Supabase client sử dụng environment variables được cung cấp. Client này được sử dụng để query database, authenticate, upload files, v.v.",
  ),
  ...
  normalText(
    "Đoạn mã này là Server Component cho trang login. Nó kiểm tra xem user đã authenticated chưa, nếu rồi thì redirect đến dashboard.",
  ),
  ...
  normalText(
    "API route này xử lý tạo đơn đặt phòng mới. Nó validate dữ liệu, insert vào database, và return booking id.",
  ),

  new Paragraph({ text: "", spacing: { line: 360 } }),
  centeredText(
    "[PLACEHOLDER: Hình A.1 - Supabase Client Initialization screenshot]",
  ),
...
sections.push([
  ...
  normalText(
    "Phần này chứa các hình ảnh giao diện chính của hệ thống StaySaga.",
  ),
  ...
  normalText(
    "Giao diện đăng nhập/đăng ký được thiết kế đơn giản, thân thiện. User có thể nhập email, password, và click login hoặc signup.",
  ),
  ...
  normalText(
    "Dashboard hiển thị thông tin tổng quan, stats, charts, và các actions nhanh.",
  ),
  ...
  normalText(
    "Host có thể xem danh sách phòng, thêm phòng mới, chỉnh sửa, xóa.",
  ),
  ...
  normalText(
    "Guest có thể tìm kiếm phòng theo ngày, xem chi tiết phòng, và tạo đơn đặt.",
  ),
  ...
  normalText(
    "Host có thể xem danh sách đơn đặt, chỉnh sửa trạng thái, hủy đơn. Guest có thể xem booking của họ.",
  ),
  ...
  normalText(
    "Hệ thống hoạt động trên production domain với HTTPS được cấu hình đúng.",
  ),
]);
...
sections.push([
  ...
  normalText(
    "Chứng cứ monitoring được cài đặt, logs collected, errors tracked.",
  ),
]);
...
sections.push([
  ...
  normalText(
    "Phần này chứa các chứng cứ (screenshots) của quá trình sử dụng AI tools trong phát triển.",
  ),
  ...
  normalText(
    "Chứng cứ prompt yêu cầu AI thiết kế database schema, response từ AI.",
  ),
  ...
  normalText(
    "Chứng cứ prompt yêu cầu AI tạo Dockerfile, response multi-stage.",
  ),
  ...
  normalText(
    "Chứng cứ prompt yêu cầu viết documentation section, AI response.",
  ),
...
sections.push([
  ...
  normalText(
    "Phần này chứa các câu hỏi và trả lời dự kiến có thể được hỏi trong phòng viva.",
  ),
  ...
  normalText(
    "A: App Router là cơ chế routing mới của Next.js, sử dụng folder-based routing trong thư mục app/. Khác biệt với Pages Router: App Router hỗ trợ Server Components, Layouts, RLS-like features. App Router có performance tốt hơn do tree-shaking và code splitting.",
  ),

  heading3("Q2: Server Components và Client Components khác nhau như thế nào?"),
  normalText(
    "A: Server Components được render trên server, giảm JS gửi đến client, có thể truy cập database trực tiếp. Client Components được render ở client, dùng React hooks, interactive. Default là Server Component, cần 'use client' để client.",
  ),

  heading3("Q3: Làm sao sử dụng Server Actions?"),
  normalText(
    "A: Server Actions là functions được định nghĩa với 'use server' directive, có thể được gọi từ Client Components mà không cần tạo API route. Dùng để mutation data (create, update, delete) một cách tiện lợi.",
  ),
  ...
  normalText(
    "A: RLS là PostgreSQL feature cho phép kiểm soát truy cập ở mức row database. Cần vì đảm bảo bảo mật ở database level, ngay cả khi application code bị hack. Với RLS, host chỉ xem phòng của họ, guest chỉ xem booking của họ.",
  ),

  heading3("Q5: Làm sao enable RLS trong Supabase?"),
  normalText(
    "A: Trong Supabase console, chọn table, click 'Enable RLS'. Sau đó create policies cho mỗi operation (SELECT, INSERT, UPDATE, DELETE). Ví dụ: CREATE POLICY 'host_rooms' ON rooms USING (auth.uid() = host_id).",
  ),

  heading3("Q6: JWT token là gì? Cách hoạt động?"),
  normalText(
    "A: JWT (JSON Web Token) là token encode thông tin người dùng. Supabase tạo JWT khi user login. Token gửi ở header Authorization, Supabase SDK xác minh token trước mỗi request. Nếu invalid, request bị reject.",
  ),
  ...
  normalText(
    "A: Docker là containerization platform, đóng gói app + dependencies vào container. Tại sao: Consistency (chạy giống nhau ở mọi nơi), Isolation (dependencies tách biệt), Deployment dễ dàng (push image, chạy container), Scaling (tạo multiple containers).",
  ),

  heading3("Q8: Multi-stage Dockerfile là gì?"),
  normalText(
    "A: Multi-stage Dockerfile có nhiều FROM statements. Stage 1 cài dependencies, Stage 2 build app, Stage 3 chạy. Tại sao: giảm final image size (chỉ copy binary, không cần build tools).",
  ),

  heading3("Q9: Docker Compose là gì? Sử dụng khi nào?"),
  normalText(
    "A: Docker Compose define multiple containers (app, db, nginx) trong 1 file. Chạy: docker-compose up. Sử dụng khi: cần nhiều services, cần run local, cần orchestrate multiple containers.",
  ),

  heading3("Q10: VPS deployment steps?"),
  normalText(
    "A: 1) SSH vào VPS. 2) Cài Docker. 3) Clone repo. 4) Tạo .env file. 5) docker-compose up -d. 6) Setup domain DNS. 7) Setup SSL. 8) Verify ứng dụng chạy.",
  ),
  ...
  normalText(
    "A: Chính: users (hosts, guests), rooms, bookings, payments, room_images, customers. Relationships: users 1-M rooms, rooms 1-M bookings, bookings 1-M payments, rooms 1-M room_images.",
  ),

  heading3("Q12: Bảng rooms cần fields nào?"),
  normalText(
    "A: id (PK), host_id (FK), name, description, price, capacity, bedrooms, bathrooms, amenities (JSONB), created_at, updated_at.",
  ),

  heading3("Q13: Bảng bookings cần fields nào?"),
  normalText(
    "A: id (PK), room_id (FK), guest_id (FK), check_in, check_out, status (pending/confirmed/cancelled), total_price, created_at, updated_at.",
  ),
  ...
  normalText(
    "A: Password được hash (bcrypt, argon2) trước khi lưu. Never store plain text password. Supabase Auth tự động hash.",
  ),

  heading3("Q15: CORS là gì? Tại sao cần?"),
  normalText(
    "A: CORS (Cross-Origin Resource Sharing) kiểm soát requests từ domain khác. Cần để ngăn unauthorized access từ external websites. Setup: Allow specific origins, methods, headers.",
  ),

  heading3("Q16: Input validation tại sao quan trọng?"),
  normalText(
    "A: Validate input để ngăn SQL injection, XSS, malformed data. Validate trước khi insert vào database. Dùng schema validation libraries (Zod, Yup).",
  ),
  ...
  normalText(
    "A: 1) Database indexing. 2) Query optimization. 3) Caching (Redis). 4) Image compression. 5) Code splitting. 6) Lazy loading. 7) CDN for static files. 8) Monitoring & profiling.",
  ),

  heading3("Q18: Image optimization như thế nào?"),
  normalText(
    "A: Compress (reduce size), Resize (responsive sizes), Format (WebP, AVIF), Lazy load, Progressive load. Next.js Image component tự động optimize.",
  ),

  heading3("Q19: Database query optimization?"),
  normalText(
    "A: 1) Add indexes. 2) Select specific columns (not SELECT *). 3) Use JOINs instead of multiple queries. 4) Pagination for large datasets. 5) Cache frequently accessed data.",
  ),
  ...
  normalText(
    "A: Type safety (catch errors early), Better IDE autocomplete, Self-documenting code, Easier refactoring, Reduce runtime errors. Trade-off: compilation step, learning curve.",
  ),

  heading3("Q21: Generic types là gì? Ví dụ?"),
  normalText(
    "A: Generics allow reusable code với different types. Ví dụ: function fetchData<T>(url: string): Promise<T>. Lợi ích: type safety, reusability.",
  ),
  ...
  normalText(
    "A: Unit testing (individual functions), Integration testing (modules together), E2E testing (full user flow), Performance testing, Security testing.",
  ),

  heading3("Q23: Test coverage là gì?"),
  normalText(
    "A: Percentage of code được test. Cao coverage = tốt, nhưng không guarantee chất lượng. Target: 80%+ coverage.",
  ),
  ...
  normalText(
    "A: Code generation, Debugging, Documentation, Learning, Optimization. Always verify AI output, không sử dụng mù quáng.",
  ),

  heading3("Q25: Hạn chế của AI?"),
  normalText(
    "A: Không luôn chính xác, có thể bugs, training data lỗi thời, không hiểu context, vi phạm copyright.",
  ),
  ...
  normalText(
    "A: Logs collection (stdout/stderr), Error tracking (Sentry), Performance monitoring (DataDog), Uptime monitoring (Pingdom), Database monitoring.",
  ),

  heading3("Q27: Deployment strategy?"),
  normalText(
    "A: Blue-green deployment (2 envs, switch traffic), Canary deployment (gradual rollout), Rolling deployment (update gradually), Rollback strategy.",
  ),

  heading3("Q28: Common production issues?"),
  normalText(
    "A: Out of memory, Database connection pool exhausted, Disk space full, SSL certificate expired, DNS misconfigured, Rate limiting exceeded.",
  ),
  ...
  normalText(
    "A: Planning: 1 week, Development: 4 weeks, Testing: 1.5 weeks, Deployment: 3-4 days. Total: ~7 weeks. Challenges: scope creep, integration issues, testing bugs.",
  ),

  heading3("Q30: Lessons learned?"),
  normalText(
    "A: Start with clear requirements, Use version control properly, Test early and often, Monitor from day 1, Document as you code, Use AI wisely, Plan deployment early.",
  ),

  heading3("Q31: Future improvements?"),
  normalText(
    "A: Payment integration, Real-time features, Mobile app, AI pricing, Multi-language, Calendar sync, Advanced analytics, Marketplace expansion.",
  ),

  heading3("Q32: System limitations?"),
  normalText(
    "A: Payment: manual confirmation only, Real-time: email only, Language: Vietnamese only, Mobile: responsive web only, Analytics: basic only.",
  ),

  heading3("Q33: Security audit được thực hiện?"),
  normalText(
    "A: Code review, Dependency scanning (npm audit), SQL injection testing, XSS testing, CORS testing, Authentication/authorization testing.",
  ),

  heading3("Q34: Scalability plan?"),
  normalText(
    "A: Database: read replicas, sharding. Cache: Redis. Load balancing: nginx. Microservices: APIs. Monitoring: proactive alerts.",
  ),

  heading3("Q35: Cost estimation?"),
  normalText(
    "A: VPS: $5-20/month, Database: included in Supabase, Storage: based on usage, Domain: $10-15/year, SSL: free (Let's Encrypt).",
  ),
]);
...
Packer.toBuffer(doc).then((buffer) => {
  ...
  console.log(
    "📊 Nội dung: PHỤ LỤC A-E (Source code, Interface images, Deployment evidence, AI evidence, Q&A)",
  );
});
