...
export default async function AdminSettingsPage({ searchParams }: SettingsPageProps) {
  ...
  const heroTitle = settings.hero_title || "Khám phá những điểm lưu trú tuyệt vời nhất";
  const heroSubtitle = settings.hero_subtitle || "Đặt homestay, khách sạn và trải nghiệm nghỉ dưỡng tiện lợi.";
  const featuredDestinations = settings.featured_destinations || "da-lat,ha-noi,da-nang,saigon";
  const propertyTypes = settings.property_types || "Homestay, Khách sạn, Resort, Biệt thự, Căn hộ";
  const defaultAmenities = settings.default_amenities || "Wifi, Điều hòa, Hồ bơi, Chỗ đỗ xe, Bếp, Máy giặt";
    ...
    >
      ...
      {params.error && (
        <div className="mb-6 rounded-xl border border-rose-200 bg-rose-50 p-4 text-sm font-bold text-rose-800 shadow-sm flex items-center gap-2">
          ...
          Không thể cập nhật cấu hình. Vui lòng kiểm tra lại quyền Supabase RLS. (Chế độ xem trước khả dụng)
        </div>
      )}
        ...
        <div className="lg:col-span-2">
          <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
            <h3 className="text-lg font-extrabold text-slate-900 mb-6 flex items-center gap-2 border-b pb-4">
              <Sparkles className="h-5 w-5 text-rose-500" /> Cấu hình Website Frontend
            </h3>

            <form action={updateWebsiteSettings} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-xs font-black uppercase tracking-wider text-slate-700 mb-2">Tên thương hiệu (Website Name)</label>
                  <input
                    name="site_name"
                    required
                ...
                <div>
                  <label className="block text-xs font-black uppercase tracking-wider text-slate-700 mb-2">Tiêu đề Homepage (Hero Title)</label>
                  <input
                    name="hero_title"
                    required
              ...
              <div>
                <label className="block text-xs font-black uppercase tracking-wider text-slate-700 mb-2">Mô tả Homepage (Hero Subtitle)</label>
                <textarea
                  name="hero_subtitle"
                  rows={3}
              ...
              <div className="border-t pt-6">
                <h4 className="text-sm font-extrabold text-slate-900 mb-4">Danh mục & Từ khóa</h4>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="md:col-span-1">
                    <label className="block text-xs font-black uppercase tracking-wider text-slate-700 mb-2">Thành phố nổi bật (Slugs)</label>
                    <input
                    ...
                    />
                    <p className="mt-1.5 text-[10px] text-slate-500 leading-normal">Cách nhau bằng dấu phẩy. Sử dụng slug viết liền không dấu.</p>
                  </div>

                  <div className="md:col-span-1">
                    <label className="block text-xs font-black uppercase tracking-wider text-slate-700 mb-2">Loại chỗ nghỉ</label>
                    <input
                      name="property_types"
                    ...
                    />
                    <p className="mt-1.5 text-[10px] text-slate-500 leading-normal">Các loại chỗ nghỉ hiển thị khi đối tác tạo tin đăng mới.</p>
                  </div>

                  <div className="md:col-span-1">
                    <label className="block text-xs font-black uppercase tracking-wider text-slate-700 mb-2">Tiện nghi mặc định</label>
                    <input
                      ...
                      className="w-full rounded-lg border border-slate-300 bg-white px-4 py-2.5 text-xs font-bold text-slate-950 placeholder:text-slate-400 focus:border-rose-500 focus:ring-2 focus:ring-rose-100 focus:outline-none transition-all"
                    />
                    <p className="mt-1.5 text-[10px] text-slate-500 leading-normal">Danh sách tiện nghi gợi ý phổ biến cho toàn bộ hệ thống.</p>
                  </div>
                </div>
              </div>

              <div className="border-t pt-6">
                <label className="block text-xs font-black uppercase tracking-wider text-slate-700 mb-2">Banner Trang chủ (Hero Background Image)</label>
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
            <h3 className="text-sm font-black uppercase tracking-wider text-slate-700 mb-4 flex items-center gap-1.5">
              <ImageIcon className="h-4 w-4 text-slate-400" /> Xem trước giao diện chính
            </h3>
            ...
            <div className="relative rounded-lg overflow-hidden border border-slate-200 shadow-inner bg-slate-900 h-64 flex flex-col justify-between p-4 text-white">
              {heroImage ? (
                <div className="absolute inset-0 z-0">
                  <SafeImage src={heroImage} alt="Hero Banner Preview" className="object-cover w-full h-full opacity-60" />
                </div>
              ) : (
                <div className="absolute inset-0 bg-gradient-to-br from-rose-900/60 to-indigo-900/65 z-0 flex items-center justify-center">
                  <span className="text-[10px] text-white/50 uppercase font-black tracking-widest">Không có banner (Dùng màu gradient mặc định)</span>
                </div>
              )}

              {/* Logo / Header strip */}
              <div className="relative z-10 flex justify-between items-center w-full">
                <span className="text-sm font-black tracking-tight">{brandName}</span>
            ...
            <div className="mt-4 border-t pt-4 space-y-3">
              <div>
                <p className="text-[11px] font-bold text-slate-500">Thành phố nổi bật</p>
                <div className="flex flex-wrap gap-1 mt-1.5">
                  {featuredDestinations.split(",").map((slug: string) => (
                    <span key={slug} className="text-[10px] font-mono font-bold bg-slate-100 text-slate-700 px-2 py-0.5 rounded">
                      {slug.trim()}
                    </span>
                  ))}
                </div>
              ...
              <div>
                <p className="text-[11px] font-bold text-slate-500">Loại chỗ nghỉ hỗ trợ</p>
                <div className="flex flex-wrap gap-1 mt-1.5">
                  {propertyTypes.split(",").map((t: string) => (
                    <span key={t} className="text-[10px] font-semibold bg-rose-50 text-rose-700 px-2 py-0.5 rounded">
                      {t.trim()}
                    </span>
                  ))}
                </div>
              ...
              <div>
                <p className="text-[11px] font-bold text-slate-500">Tiện nghi mặc định</p>
                <p className="text-xs text-slate-700 mt-1 leading-normal font-medium">{defaultAmenities}</p>
              </div>
            </div>
          </div>
        </div>
