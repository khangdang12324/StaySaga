...
export function HomestaysClient({ hostHotels = [] }: { hostHotels?: Hotel[] }) {
  return (
    <Suspense fallback={<HomestaysLoading />}>
      <HomestaysPageContent hostHotels={hostHotels} />
    </Suspense>
  );
}

function HomestaysPageContent({ hostHotels }: { hostHotels: Hotel[] }) {
  const searchParams = useSearchParams();
  const locationParam = searchParams.get("location")?.trim() || "";
  const allHotels = useMemo(() => [...hostHotels, ...getAllHotels()], [hostHotels]);
  const cities = useMemo(
    () => Array.from(new Set([...supportedCities, ...getAvailableCities(), ...hostHotels.map((hotel) => hotel.city)])),
    [hostHotels],
  );
...
function SearchResultCard({
  ...
  const targetSlug = canonical || hotel.slug;
  const query = params.toString();
  const href = targetSlug
    ? `/homestays/${targetSlug}${query ? `?${query}` : ""}`
