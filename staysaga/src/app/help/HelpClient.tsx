...
export default function HelpClient({
  ...
  const [selectedBookingId, setSelectedBookingId] = useState(initialBookingId || (bookings[0]?.id || ""));
  ...
  const faqs = [
    ...
    {
      q: t("Chính sách hủy phòng như thế nào?", "What is the cancellation policy?"),
      a: t(
    ...
    {
      q: t("Tôi có thể thay đổi ngày đặt phòng không?", "Can I change my booking dates?"),
      a: t(
    ...
    {
      q: t("Phương thức thanh toán nào được chấp nhận?", "What payment methods are accepted?"),
      a: t(
  ...
  const selectedBookingCheckIn = activeBooking?.check_in_date ? new Date(activeBooking.check_in_date) : null;
  const selectedBookingCheckOut = activeBooking?.check_out_date ? new Date(activeBooking.check_out_date) : null;
    ...
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans pb-20">
      ...
      <header className="bg-rose-600 pt-3.5 pb-3.5 shadow-md">
        <div className="max-w-4xl mx-auto px-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3 text-white text-sm font-bold">
            <Link href="/bookings" className="hover:bg-rose-700 p-2 px-3 rounded-lg cursor-pointer transition-colors text-white">
      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
        ...
        <div className="flex items-center gap-2 text-xs sm:text-sm text-slate-500 mb-6 font-semibold">
          <Link href="/" className="text-rose-650 hover:text-rose-750 transition-colors">
            {t("Trang chủ", "Home")}
          </Link>
          <ChevronRight className="w-3.5 h-3.5 text-slate-400 shrink-0" />
          <Link href="/help" className="text-rose-650 hover:text-rose-750 transition-colors">
            {t("Trung tâm trợ giúp", "Customer Service")}
          </Link>
          {activeBooking && (
            <>
              <ChevronRight className="w-3.5 h-3.5 text-slate-400 shrink-0" />
              <span className="text-slate-700 truncate">{t("Yêu cầu về đơn đặt phòng", "Reservation Help")}</span>
            </>
          )}
        </div>
        ...
        {showWarning && (
          <div className="bg-white border-2 border-amber-500 rounded-xl p-5 shadow-sm relative overflow-hidden mb-6 flex gap-4 items-start">
            ...
            <div className="flex-1 pr-6 text-slate-800 text-xs sm:text-sm leading-relaxed">
              <h4 className="font-extrabold text-slate-900 mb-1">{t("Luôn an toàn online", "Always safe online")}</h4>
              <p>
                {t(
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm mb-6 flex flex-col sm:flex-row justify-between sm:items-center gap-4">
            <div>
              <h3 className="font-bold text-sm sm:text-base text-slate-900">
                {t("Đơn đặt phòng đang được hỗ trợ:", "Booking you need help with:")}
              </h3>
              <p className="text-xs text-slate-400 mt-0.5">
                {t("Chọn đơn đặt phòng của bạn bên dưới để nhận trợ giúp phù hợp.", "Select your reservation below to receive specific support.")}
              </p>
            </div>
            <div className="relative min-w-[200px]">
              ...
              >
                <option value="">-- {t("Chọn đặt phòng", "Select booking")} --</option>
                {bookings.map((b) => (
                  <option key={b.id} value={b.id}>
                    {b.homestay?.name || b.homestay?.city || "StaySaga Stay"} ({(b.booking_code || b.id).slice(0, 8).toUpperCase()})
                  </option>
                ))}
              </select>
        ...
        {activeBooking ? (
          <div className="space-y-6">
              ...
              <div className="p-5 flex items-start sm:items-center gap-4 relative">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-slate-500 font-semibold mt-0.5">
                    {formattedDates(selectedBookingCheckIn, selectedBookingCheckOut)}
                  </p>
                </div>
                <button 
                  onClick={() => window.location.href = `/bookings/${activeBooking.id}`}
              ...
              {submittedText && (
                <div className="px-5 pb-4 border-t border-slate-100 pt-3">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm text-slate-800">{submittedText}</span>
                  </div>
                </div>
              )}
            </div>
            ...
            {!submittedText ? (
              <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
                ...
                <p className="text-xs text-slate-450 font-semibold mb-4 leading-relaxed">
                  {t("Mô tả câu hỏi hoặc yêu cầu cụ thể của bạn.", "Describe your question or specific request.")}
                </p>
                ...
                <div className="space-y-4">
                  <textarea
                    ...
                    placeholder={t("Thêm vài chi tiết để giúp chúng tôi biết mình nên làm gì tiếp theo...", "Add details to help us know what to do next...")}
            ...
            <div>
              ...
              <p className="text-xs text-slate-450 font-semibold mb-4 leading-relaxed">
                {t("Chọn 1 trong những lựa chọn dưới đây và chúng tôi sẽ hướng dẫn bạn bước kế tiếp.", "Select an option below and we'll guide you through the next steps.")}
              </p>
              ...
              <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden divide-y divide-slate-100">
                ...
                <button
                  onClick={() => {
                    const contactsEl = document.getElementById("support-contacts");
                    if (contactsEl) {
                      contactsEl.scrollIntoView({ behavior: "smooth" });
                      toast.success(t("Vui lòng liên hệ trực tiếp với chúng tôi qua các phương thức bên dưới.", "Please reach out via support channels below."));
                    }
                  }}
          ...
          <div className="space-y-6 animate-fade-in">
            <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
              ...
              <p className="text-xs text-slate-450 font-semibold mb-6">
                {t("Tìm kiếm nhanh câu trả lời hoặc chọn một đặt phòng của bạn ở trên để nhận hỗ trợ cụ thể.", "Find answers to frequently asked questions or select a booking above to receive customized help.")}
              </p>
                ...
                {faqs.map((faq, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        <span>{faq.q}</span>
                        <ChevronRight className={`w-4 h-4 text-slate-450 transition-transform ${isOpen ? "rotate-90 text-rose-500" : ""}`} />
                      </button>
                      {isOpen && (
        ...
        <div id="support-contacts" className="mt-8 pt-8 border-t border-slate-200">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            >
              ...
              <div>
                <h3 className="font-extrabold text-sm text-slate-900 mb-0.5">{t("Chat trực tuyến", "Live Chat")}</h3>
                <p className="text-xs text-slate-450 font-semibold leading-relaxed">
                  {t("Trò chuyện và nhận câu trả lời nhanh từ chúng tôi hoặc chủ nhà 24/7.", "Message us or property host directly for 24/7 support.")}
                </p>
              </div>
            </div>
            ...
            >
              ...
              <div>
                <h3 className="font-extrabold text-sm text-slate-900 mb-0.5">{t("Gửi email", "Email support")}</h3>
                <p className="text-xs text-slate-450 font-semibold leading-relaxed">
                  support@staysaga.com <br />
                  {t("Đội ngũ kỹ thuật hỗ trợ giải đáp trong vòng 24h.", "Our technical team will respond within 24 hours.")}
                </p>
              </div>
            </a>
